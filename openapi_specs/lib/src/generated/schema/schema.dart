...
class GithubRepositoryOut with _$GithubRepositoryOut {
  ...
  const factory GithubRepositoryOut({
    ///
    @Default(GithubRepositoryOutType.github) GithubRepositoryOutType type,
...
class BatchJobOut with _$BatchJobOut {
  ...
  const factory BatchJobOut({
    ...
    ///
    @Default(Object.batch) Object object,
...
class ChatCompletionRequest with _$ChatCompletionRequest {
  ...
  const factory ChatCompletionRequest({
    ...
    @JsonKey(name: 'tool_choice') @Default(ToolChoice()) ToolChoice toolChoice,
...
class DeltaMessage with _$DeltaMessage {
  ...
  const factory DeltaMessage({
    ...
    /// No Description
    @_DeltaMessageContentConverter()
    @JsonKey(includeIfNull: false)
    DeltaMessageContent? content,
...
// ==========================================
// CLASS: DeltaMessageContentNull
// ==========================================

/// No Description
@freezed
class DeltaMessageContentNull with _$DeltaMessageContentNull {
  const DeltaMessageContentNull._();

  /// Factory constructor for DeltaMessageContentNull
  const factory DeltaMessageContentNull() = _DeltaMessageContentNull;

  /// Object construction from a JSON representation
  factory DeltaMessageContentNull.fromJson(Map<String, dynamic> json) =>
      _$DeltaMessageContentNullFromJson(json);
...
// ==========================================
// CLASS: DeltaMessageContent
// ==========================================

/// No Description
@freezed
sealed class DeltaMessageContent with _$DeltaMessageContent {
  const DeltaMessageContent._();

  const factory DeltaMessageContent.array(
    List<ContentChunk> value,
  ) = _UnionDeltaMessageContentArray;

  const factory DeltaMessageContent.string(
    String value,
  ) = _UnionDeltaMessageContentString;

  /// Object construction from a JSON representation
  factory DeltaMessageContent.fromJson(Map<String, dynamic> json) =>
      _$DeltaMessageContentFromJson(json);
}

/// Custom JSON converter for [DeltaMessageContent]
class _DeltaMessageContentConverter
    implements JsonConverter<DeltaMessageContent?, Object?> {
  const _DeltaMessageContentConverter();

  @override
  DeltaMessageContent? fromJson(Object? data) {
    if (data == null) {
      return null;
    }
    if (data is List && data.every((item) => item is ContentChunk)) {
      return DeltaMessageContent.array(data.cast());
    }
    if (data is String) {
      return DeltaMessageContent.string(data);
    }
    throw Exception(
      'Unexpected value for DeltaMessageContent: $data',
    );
  }

  @override
  Object? toJson(DeltaMessageContent? data) {
    return switch (data) {
      _UnionDeltaMessageContentArray(value: final v) => v,
      _UnionDeltaMessageContentString(value: final v) => v,
      null => null,
    };
  }
}
